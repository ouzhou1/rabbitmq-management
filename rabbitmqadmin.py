    ...
    def configuration_file_validation(self,dfs):
        ...
        for key in definitions.keys():
            ...
            if key in CHECKTABLE.keys():
                ...
                check_method(self,definitions[key])
                self.verbose("OK!")

    def check_vhosts(self,check_content):
        for item in check_content:
            ...
            # assert_usage(check_field[0]["name"] == resp_json["name"],"Vhost name error")

    def check_queues(self,check_content):
        for item in check_content:
            # mandatory keys
            path = '/api/queues/%s/%s' % (item["vhost"],item["name"])
            body_dict = {}
            # optional keys
            optional_keys = ["auto_delete","durable","arguments","node"]
            for opk in optional_keys:
                if opk in item:
                    body_dict[opk] = item[opk]
            body = json.dumps(body_dict)
            resp_json = json.loads(self.http("GET", path, body))

    def check_exchanges(self,check_content):
        for item in check_content:
            # mandatory keys
            path = '/api/exchanges/%s/%s' % (item["vhost"],item["name"])
            body_dict = {}
            body_dict["type"] = item["type"]
            # optional keys
            optional_keys = ["auto_delete","durable","internal","arguments"]
            for opk in optional_keys:
                if opk in item:
                    body_dict[opk] = item[opk]
            body = json.dumps(body_dict)
            resp_json = self.http("PUT", path, body)

    def check_bindings(self,check_content):
        for item in check_content:
            # mandatory keys
            path = '/api/bindings/%s/e/%s/%s/%s' % (item["vhost"],item["source"],item["destination_type"][0],item["destination"])
            body_dict = {}
            # optional keys
            optional_keys = ["routing_key","arguments"]
            for opk in optional_keys:
                if opk in item:
                    body_dict[opk] = item[opk]
            body = json.dumps(body_dict)
            resp_json = self.http("POST", path, body)
